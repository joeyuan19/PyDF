...
def py_dict_to_pdf_dict(py_dict,is_stream=False):
    ...
    for key in py_dict:
        if is_stream:
            if key != 'stream' and key != 'stream_decoded':
                buf += key + " " + py_obj_to_pdf_obj(py_dict[key]) + " "
        else:
            buf += str(key) + " " + py_obj_to_pdf_obj(py_dict[key]) + " "
    buf += ">>"
    if is_stream:
        buf += "\nstream\n"
        buf += py_dict['stream'] + "\n"
...
def linearize_whitespace(string):
    ...
    for delimeter in PDF_DELIMETERS:
        temp = temp.replace(delimeter,' '+delimeter)
...
def get_pdf_date():
    ...
    time = re.sub(r'[^\d]','',str(time)[:str(time).find('.')])
    return 'D:'+time

def format_and_pad(n,padding_digits=10,pad_with=' '):
    return ''.join(pad_with for i in range(padding_digits-len(str(n))))+str(n) 

def format_pos_number(n,padding_digits=10,pad_with='0'):
    return format_and_pad(n,padding_digits,pad_with) 
